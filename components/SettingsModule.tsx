...
import { Save, Info, Key, FolderOpen, ShieldCheck, Globe } from 'lucide-react';
...
const SettingsModule: React.FC<SettingsModuleProps> = ({ settings, onSave, onKeyChange }) => {
  ...
  // Detect if values are coming from environment variables (VPS)
    ...
    <div className="max-w-3xl mx-auto space-y-6 pb-20">
      <section className="glass p-8 rounded-2xl border border-white/10">
        ...
        <div className="mb-8 p-6 bg-blue-500/5 border border-blue-500/20 rounded-2xl space-y-3">
          ...
          <p className="text-[11px] text-slate-400">
            When you configure variables in Coolify, these fields will be automatically populated for all users.
          </p>
        </div>
        ...
        <form onSubmit={handleSave} className="space-y-6">
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="Injected from environment..."
            />
          </div>
      ...
      <div className="p-4 bg-white/5 border border-white/10 rounded-xl flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-slate-400">App Origin: <strong className="text-white">{window.location.origin}</strong></span>
        </div>
